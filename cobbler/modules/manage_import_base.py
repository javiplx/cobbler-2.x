...
class ImportBaseManager:
    ...
    def check_for_signature(self,path,cli_breed):
       self.logger.info("scanning %s for a %s-based distro signature" % (path,self.__breed_str__))
       for signature in self.signatures:
           d = os.path.join(path,signature)
           if os.path.exists(d):
               self.logger.info("Found a %s compatible signature: %s" % (self.__breed_str__,signature))
    ...
    def set_breed_from_directory ( self ) :
        """If no breed was specified on the command line, this method is called"""
        self.breed = self.__breed_str__
    ...
    def get_install_tree(self, distro, base):
        raise NotImplemented( "needs to be implemented" , self , "get_install_tree" )
    ...
    def valid_initrd(self,name):
        raise NotImplemented( "needs to be implemented" , self , "valid_initrd" )

    def valid_kernel(self,name):
        raise NotImplemented( "needs to be implemented" , self , "valid_kernel" )
    ...
    def distro_adder(self,distros_added,dirname,fnames):
        ...
        for x in fnames:
            ...
            if self.valid_initrd(x) and x != "initrd.size":
                if x.find("PAE") == -1:
                    initrd = os.path.join(dirname,x)
                else:
                    pae_initrd = os.path.join(dirname, x)

            if self.valid_kernel(x) and x.find("initrd") == -1:
                if x.find("PAE") == -1:
    ...
    def get_name_from_path(self,dirname):
        raise NotImplemented( "needs to be implemented" , self , "get_name_from_path" )
